...
calculate_tseprofexh <- function(count_matrix) {
  # Validate the input
  ...
  # Add proliferation genes
  proliferation_genes <- c(
    "ANLN", "TOP2A", "KIF11", "MK167", "CDK1", "CDKN3",
    "FOXM1", "BUB1", "KIAA0101", "MAD2L1", "MELK", "CCNB2"
  )

  exhaustedtcell <- c("CPS1", "FOSL1", "GJB3", "HLA-DOB", "IGFBP1", "KLK11", "KRT6A", "KRT81")
  genesTargetTherapy[["Proliferation"]] <- proliferation_genes

  genesTargetTherapy[["ExhTCell"]] <- exhaustedtcell
  ...
  ResultsGeneSignatures <- ResultsGeneSignatures %>%
    ...
    dplyr::select(sampleId, TSE_Value, Tcellsig, Stromalsig, Proliferation, ExhTCell)
